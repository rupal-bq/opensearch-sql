...
package org.opensearch.sql.cloudwatch.storage.querybuilder;
  ...
  public static String build(List<NamedAggregator> namedAggregatorList,
    ...
    if (namedAggregatorList.size() > 1) {
      throw new RuntimeException(
          "CloudWatch Catalog doesn't multiple aggregations in stats command");
    }

    if (!allowedStatsFunctions
        .contains(namedAggregatorList.get(0).getFunctionName().getFunctionName())) {
      throw new RuntimeException(String.format(
          "CloudWatch Catalog only supports %s aggregations.", allowedStatsFunctions));
    }
